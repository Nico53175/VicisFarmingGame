...
public class CharacterAppearance : MonoBehaviour
{
    ...
    public void InteractionUpdate(Transform other)
    {
        movement.x = 0;
        movement.y = 0;
        animationUpdate();
        ...
        float dx = otherPosition.x - tf.position.x;
        float dy = otherPosition.y - tf.position.y;
        ...
        else
        {
            if (dy > 0)
            {
                // Player is directly behind NPC
                animator.SetInteger("orientation", 0);
            }
            else
            {
                // Player is directly in front NPC
                animator.SetInteger("orientation", 4);
            }
        }
    }
